...
import { ExtrasData } from '../utils/data'

export default function ExtrasSlider(){

    const slideLeft = () => {
        var slider = document.getElementById('sliderExtras');
    ...
    return(
        <div>
            <h1 className='mx-4 font-bold text-4xl text-center mt-10'>Extras</h1>
            ...
            <div id='sliderExtras' className='w-full h-full overflow-x-scroll scroll whitespace-nowrap scroll-smooth scrollbar-hide pb-5'>
                {
                    ExtrasData.map((data) => {
                        return(
                            <div key={data.key} className="glassmorph inline-block p-2 cursor-pointer w-64 lg:w-96 mx-3 rounded-lg">
                                ...
                                <div className="font-Koho px-5">
                                    ...
                                    <button class="inline-flex text-sm items-center justify-center p-0.5 overflow-hidden font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-blue-500 to-orange-400 group-hover:from-pink-500 group-hover:to-orange-400 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800 mx-auto">
                                        <a href={`/extras/${data.key}`} class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
                                            Discover
                                        </a> 
                                    </button>
                                    </div>
